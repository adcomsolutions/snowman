...
const squashObjs = (objects) => objects
      .reduce((memo, nextObj) => ({
        ...memo,
        ...nextObj
      }), {})
...
const baseify = (libName) =>
      path.basename( libName, jsExt )

const externalify = (libName) =>
...
const filterLibs = (bundleName, libs) => libs
  .map(externalify)
  .filter((lib) =>
          getLastDir(lib) !== bundleName
         )
